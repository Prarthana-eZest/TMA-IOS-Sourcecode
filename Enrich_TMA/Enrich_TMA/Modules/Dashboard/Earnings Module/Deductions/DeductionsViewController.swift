...
protocol DeductionsDisplayLogic: class
{
  func displaySomething(viewModel: Deductions.Something.ViewModel)
}

class DeductionsViewController: UIViewController, DeductionsDisplayLogic
{
  var interactor: DeductionsBusinessLogic?
  var router: (NSObjectProtocol & DeductionsRoutingLogic & DeductionsDataPassing)?

    @IBOutlet private weak var tableView: UITableView!
    var dateRangeType : DateRangeType = .mtd
    var deductionsDateRange:DateRange = DateRange(Date.today.lastYear(), Date.today)
  ...
  private func setup()
  {
    ...
    let interactor = DeductionsInteractor()
    let presenter = DeductionsPresenter()
    let router = DeductionsRouter()
  ...
  // MARK: View lifecycle// MARK: View lifecycle
    ...
    //@IBOutlet weak var nameTextField: UITextField!
      ...
      func updateDeductionsData(startDate: Date?, endDate: Date = Date().startOfDay) {
          
          EZLoadingActivity.show("Loading...", disableUI: true)
          deductionsData(startDate: startDate ?? Date.today, endDate: endDate, completion: nil)
      }
      
    func calculateTotalDeductions(dateRange: DateRange) {
          ...
          var graphDateRange = deductionsDateRange // need to change
  ...
  //            filteredFixedEarningsForGraph = nil
          ...
          let dataArray = earningsJSON?.data?.groups?.filter({EarningDetails(rawValue: $0.group_label ?? "") == EarningDetails.Deductions}) ?? []
          var amount : Int = 0
        ...
        let currentMonth = Int(Date.today.string(format: "M"))
          ...
          headerGraphData = getTotalDeductionsGraphEntry(forData: filteredFixedEarningsForGraph, dateRange: graphDateRange, dateRangeType: graphRangeType)
      }
      
      func deductionsData(startDate : Date, endDate : Date = Date().startOfDay, completion: (() -> Void)? ) {
          dataModel.removeAll()
          graphData.removeAll()
        ...
        for data in dataArray {
            ...
            for parameter in data.parameters ?? [] {
                ...
                let value = parameter.transactions?.filter({$0.month == currentMonth})
                //                value?.first?.amount
                let model = EarningsCellDataModel(earningsType: .Deductions, title: parameter.name ?? "", value: [value?.first?.amount?.roundedStringValue() ?? ""], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: deductionsDateRange)
                dataModel.append(model)
                graphData.append(getGraphEntry(parameter.name ?? "", forData: parameter.transactions, atIndex: index, dateRange: deductionsDateRange, dateRangeType: graphRangeType))
                ...
                for parameter in data.parameters ?? [] {
                    ...
                    let model = EarningsCellDataModel(earningsType: .Deductions, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: deductionsDateRange)
                    dataModel.append(model)
                    graphData.append(getGraphEntry(parameter.name ?? "", forData: parameter.transactions, atIndex: index, dateRange: deductionsDateRange, dateRangeType: graphRangeType))
        ...
        index = 0
          ...
          calculateTotalDeductions(dateRange: dateRange)
  //        for group in earningsJSON?.data?.groups ?? []{
  //            if  (EarningDetails(rawValue: group.group_label ?? "") == EarningDetails.Deductions) {
  ...
  //        //salon service
  //        //Data Model
  //        let salonServiceModel = EarningsCellDataModel(earningsType: .Deductions, title: "Salon Service", value: [Double(0.0).abbrevationString], subTitle: [""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: fixedEarningsDateRange)
  ...
  //        graphData.append(getGraphEntry(salonServiceModel.title, forData: filteredFixedEarningsForGraph, atIndex: 0, dateRange: graphDateRange, dateRangeType: graphRangeType))
      ...
      func getGraphEntry(_ title:String, forData data:[Dashboard.GetEarningsDashboard.Transaction]? = nil, atIndex index : Int, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
      {
          ...
          let graphColor = EarningDetails.Deductions.graphBarColor
  ...
  //        }
      ...
      func getTotalDeductionsGraphEntry(forData data:[Dashboard.GetRevenueDashboard.RevenueTransaction]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
      {
          let units = xAxisUnits(forDateRange: dateRange, rangeType: dateRangeType)
          let values = totalDeductionsGraphData(forData: data, dateRange: dateRange, dateRangeType: dateRangeType)
          let graphColor = EarningDetails.Deductions.graphBarColor
      ...
      func totalDeductionsGraphData(forData data:[Dashboard.GetRevenueDashboard.RevenueTransaction]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> [Double]
      {
  ...
  //        return totalFixedEarnings
      ...
      func updateDeductionsData(atIndex indexPath:IndexPath, withStartDate startDate: Date?, endDate: Date = Date().startOfDay, rangeType:DateRangeType) {
  ...
  //        }
    ...
    func doSomething()
    {
      let request = Deductions.Something.Request()
  ...
  extension DeductionsViewController: EarningsFilterDelegate {
      ...
      func actionDateFilter() {
          ...
          vc.cutomRange = deductionsDateRange
          ...
          vc.viewDismissBlock = { [unowned self] (result, startDate, endDate, rangeTypeString) in
              ...
              if(result){
                  ...
                  {
                      deductionsDateRange = DateRange(start,end)
                  }
                  updateDeductionsData(startDate: startDate ?? Date.today, endDate: endDate ?? Date.today)
              }
          }
      }
  ...
  extension DeductionsViewController: EarningDetailsDelegate {
      ...
      func actionDurationFilter(forCell cell: UITableViewCell) {
          ...
          vc.viewDismissBlock = { [unowned self] (result, startDate, endDate, rangeTypeString) in
              ...
              if result == true, startDate != nil, endDate != nil {
                  ...
                  updateDeductionsData(atIndex: indexPath, withStartDate: startDate, endDate: endDate!, rangeType: rangeType)
  ...
  extension DeductionsViewController: UITableViewDelegate, UITableViewDataSource {
