...
class DeductionsViewController: UIViewController, DeductionsDisplayLogic
{
  ...
  // MARK: View lifecycle// MARK: View lifecycle
    ...
    func calculateTotalDeductions(dateRange: DateRange) {
        ...
        headerGraphData = getTotalDeductionsGraphEntry(forData: dataArray, dateRange: graphDateRange, dateRangeType: graphRangeType)
    }
      ...
      func deductionsData(startDate : Date, endDate : Date = Date().startOfDay, completion: (() -> Void)? ) {
        ...
        for data in dataArray {
            ...
            for parameter in data.parameters ?? [] {
                ...
                let value = parameter.transactions?.filter({$0.month == currentMonth})
                //                value?.first?.amount
                let model = EarningsCellDataModel(earningsType: .Deductions, title: parameter.name ?? "", value: [value?.first?.amount?.roundedStringValue() ?? ""], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                ...
                for parameter in data.parameters ?? [] {
                    ...
                    let model = EarningsCellDataModel(earningsType: .Deductions, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                    dataModel.append(model)
                    graphData.append(getGraphEntry(parameter.name ?? "", forData: parameter.transactions, atIndex: index, dateRange: graphDateRange, dateRangeType: graphRangeType))
      ...
      func getTotalDeductionsGraphEntry(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
      {
          let units = xAxisUnitsEarnings(forDateRange: dateRange, rangeType: dateRangeType)
      ...
      func totalDeductionsGraphData(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> [Double]
      {
  ...
  //            }
          ...
          case .qtd, .ytd, .cutome:
            ...
            if let group = data?.first(where: { value in
                EarningDetails(rawValue: value.group_label ?? "") == EarningDetails.Deductions
            }) {
  ...
  //            }
      ...
      func updateDeductionsData(atIndex indexPath:IndexPath, withStartDate startDate: Date?, endDate: Date?, rangeType:DateRangeType) {
...
//        }
            ...
            headerGraphData = getTotalDeductionsGraphEntry(forData:dataArray, dateRange: dateRange, dateRangeType: rangeType)
           }
        }
