...
class IncentiveViewController: UIViewController, IncentiveDisplayLogic
{
  ...
  var router: (NSObjectProtocol & IncentiveRoutingLogic & IncentiveDataPassing)?
    ...
    var incentiveDateRange:DateRange = DateRange(Date.today.lastYear(), Date.today)
  ...
  private func setup()
  {
    ...
    override func viewDidLoad()
    {
      ...
      dateRangeType = .mtd
      updateIncentiveData(startDate: Date.today.startOfMonth)
      ...
      override func viewWillAppear(_ animated: Bool) {
          ...
          self.navigationController?.addCustomBackButton(title: "Incentive")
      }
    // MARK: Do something
    
    //@IBOutlet weak var nameTextField: UITextField!
    
      func updateIncentiveData(startDate: Date?, endDate: Date = Date().startOfDay) {
    ...
    func calculateTotalIncentive(dateRange : DateRange) {
        ...
        headerGraphData = getTotalIncentiveGraphEntry(forData: dataArray, dateRange: graphDateRange, dateRangeType: graphRangeType)
    }
        ...
        for data in dataArray {
            if(dateRangeType == .mtd){
                for parameter in data.parameters ?? [] {
                    ...
                    let value = parameter.transactions?.filter({$0.month == currentMonth})
                    //                value?.first?.amount
                    let model = EarningsCellDataModel(earningsType: .Incentive, title: parameter.name ?? "", value: [value?.first?.amount?.roundedStringValue() ?? ""], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                ...
                for parameter in data.parameters ?? [] {
                    ...
                    let model = EarningsCellDataModel(earningsType: .Incentive, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                    dataModel.append(model)
                    graphData.append(getGraphEntry(parameter.name ?? "", forData: parameter.transactions, atIndex: index, dateRange: graphDateRange, dateRangeType: graphRangeType))
      ...
      func getGraphEntry(_ title:String, forData data:[Dashboard.GetEarningsDashboard.Transaction]? = nil, atIndex index : Int, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
      {
          ...
          let graphColor = EarningDetails.Incentive.graphBarColor
      ...
      func getTotalIncentiveGraphEntry(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
      {
          let units = xAxisUnitsEarnings(forDateRange: dateRange, rangeType: dateRangeType)
          let values = totalIncentiveGraphData(forData: data, dateRange: dateRange, dateRangeType: dateRangeType)
          let graphColor = EarningDetails.Incentive.graphBarColor
      ...
      func totalIncentiveGraphData(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> [Double]
      {
  ...
  //            }
    ...
    func updateIncentiveData(atIndex indexPath:IndexPath, withStartDate startDate: Date?, endDate: Date?, rangeType:DateRangeType) {
...
//                    arr?.transactions
                    ...
                    //                let model = EarningsCellDataModel(earningsType: .Incentive, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: rangeType, customeDateRange: dateRange)
                    //                dataModel.append(model)
...
//        }
            ...
            headerGraphData = getTotalIncentiveGraphEntry(forData:dataArray, dateRange: dateRange, dateRangeType: rangeType)
           }
        }
...
//            }
  ...
  extension IncentiveViewController: EarningsFilterDelegate {
  ...
  extension IncentiveViewController: EarningDetailsDelegate {
...
//          self.tableView.endUpdates()
      ...
      func actionDurationFilter(forCell cell: UITableViewCell) {
          ...
          vc.viewDismissBlock = { [unowned self] (result, startDate, endDate, rangeTypeString) in
              ...
              if result == true, startDate != nil, endDate != nil {
                  ...
                  updateIncentiveData(atIndex: indexPath, withStartDate: startDate, endDate: endDate!, rangeType: rangeType)
  ...
  extension IncentiveViewController: UITableViewDelegate, UITableViewDataSource {
