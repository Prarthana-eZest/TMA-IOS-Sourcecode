...
protocol AwardsDisplayLogic: class
{
  func displaySomething(viewModel: Awards.Something.ViewModel)
}

class AwardsViewController: UIViewController, AwardsDisplayLogic
{
  var interactor: AwardsBusinessLogic?
  var router: (NSObjectProtocol & AwardsRoutingLogic & AwardsDataPassing)?

    @IBOutlet private weak var tableView: UITableView!
    var dateRangeType : DateRangeType = .mtd
    var awardsDateRange:DateRange = DateRange(Date.today.lastYear(), Date.today)
  ...
  private func setup()
  {
    ...
    let interactor = AwardsInteractor()
    let presenter = AwardsPresenter()
    let router = AwardsRouter()
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?)
  {
    ...
    // MARK: View lifecycle
      ...
      override func viewDidLoad()
      {
        ...
        dateRangeType = .mtd
        updateAwardsData(startDate: Date.today.startOfMonth)
      ...
      //@IBOutlet weak var nameTextField: UITextField!
        ...
        func updateAwardsData(startDate: Date?, endDate: Date = Date().startOfDay) {
    ...
    func calculateTotalAwards(dateRange: DateRange) {
        ...
        headerGraphData = getTotalAwardsGraphEntry(forData: dataArray, dateRange: graphDateRange, dateRangeType: graphRangeType)
    }
        
        func awardsData(startDate : Date, endDate : Date = Date().startOfDay, completion: (() -> Void)? ) {
            dataModel.removeAll()
            graphData.removeAll()
            ...
            for data in dataArray {
                ...
                for parameter in data.parameters ?? [] {
                    ...
                    let value = parameter.transactions?.filter({$0.month == currentMonth})
                    //                value?.first?.amount
                    let model = EarningsCellDataModel(earningsType: .Awards, title: parameter.name ?? "", value: [value?.first?.amount?.roundedStringValue() ?? ""], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                    ...
                    for parameter in data.parameters ?? [] {
                        ...
                        let model = EarningsCellDataModel(earningsType: .Awards, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: graphRangeType, customeDateRange: graphDateRange)
                        dataModel.append(model)
                        graphData.append(getGraphEntry(parameter.name ?? "", forData: parameter.transactions, atIndex: index, dateRange: graphDateRange, dateRangeType: graphRangeType))
        ...
        func getGraphEntry(_ title:String, forData data:[Dashboard.GetEarningsDashboard.Transaction]? = nil, atIndex index : Int, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
        {
            ...
            let graphColor = EarningDetails.Awards.graphBarColor
    ...
    func graphData(forData data:[Dashboard.GetEarningsDashboard.Transaction]? = nil, atIndex index : Int, dateRange:DateRange, dateRangeType: DateRangeType) -> [Double] {
      ...
      return values
        ...
        func getTotalAwardsGraphEntry(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> GraphDataEntry
        {
            let units = xAxisUnitsEarnings(forDateRange: dateRange, rangeType: dateRangeType)
            let values = totalAwardsGraphData(forData: data, dateRange: dateRange, dateRangeType: dateRangeType)
            let graphColor = EarningDetails.Awards.graphBarColor
    ...
    func totalAwardsGraphData(forData data:[Dashboard.GetEarningsDashboard.Groups]? = nil, dateRange:DateRange, dateRangeType: DateRangeType) -> [Double]
    {
...
//            }
        ...
        case .qtd, .ytd, .cutome:
            ...
            if let group = data?.first(where: { value in
                EarningDetails(rawValue: value.group_label ?? "") == EarningDetails.Awards
            }) {
...
//            }
        ...
        func updateAwardsData(atIndex indexPath:IndexPath, withStartDate startDate: Date?, endDate: Date?, rangeType:DateRangeType) {
    ...
    //                    arr?.transactions
                        ...
                        //                let model = EarningsCellDataModel(earningsType: .Awards, title: parameter.name ?? "", value: [amount.roundedStringValue()], subTitle: [parameter.comment ?? ""], showGraph: true, cellType: .SingleValue, isExpanded: false, dateRangeType: rangeType, customeDateRange: dateRange)
                        //                dataModel.append(model)
    ...
    //        }
                ...
                headerGraphData = getTotalAwardsGraphEntry(forData:dataArray, dateRange: dateRange, dateRangeType: rangeType)
               }
            }
...
//            }
      ...
      func doSomething()
      {
        let request = Awards.Something.Request()
    ...
    extension AwardsViewController: EarningDetailsDelegate {
        ...
        func actionDurationFilter(forCell cell: UITableViewCell) {
            ...
            vc.viewDismissBlock = { [unowned self] (result, startDate, endDate, rangeTypeString) in
                ...
                if result == true, startDate != nil, endDate != nil {
                    ...
                    updateAwardsData(atIndex: indexPath, withStartDate: startDate, endDate: endDate!, rangeType: rangeType)
